...
export function ProjectDetailClientWrapper({ slug }: ProjectDetailClientWrapperProps) {
  // Fetch project data client-side using tRPC
  const { data, isLoading, error } = trpc.content.getBySlug.useQuery(
    { slug, type: "project" },
    {
      retry: 1,
  ...
  // Render actual project content
  return <ProjectDetailClient slug={slug} initialData={data} />;
}
